package  kot.com.baselibrary.widgets.wheelview
...
import kot.com.baselibrary.widgets.wheelview.mode.IWheelViewMode
import kot.com.baselibrary.widgets.wheelview.mode.WheelViewRecycleMode
import kot.com.baselibrary.widgets.wheelview.mode.WheelViewStartMode
...
//    }
    ...
    fun initParams(context: Context?, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        ...
        for (i in 0..count) {
            ...
            when (attr) {
                ...
                R.styleable.WheelView_textSize -> textSize = typedArray.getDimension(attr, 30f)
    ...
    private val gestureListener = object : GestureDetector.SimpleOnGestureListener() {
        override fun onFling(e1: MotionEvent?, e2: MotionEvent?, velocityX: Float, velocityY: Float): Boolean {
            ...
            mFlingDistance = dy
            Logger.e(TAG, "onfling: scrollY = ${scrollY} --- dy = $dy -- scrollY + dy = ${scrollY + dy}")
    ...
    private val task = Runnable {
        ...
        Logger.e(TAG, "currY = $mCurrY -- mFlingDistance = $mFlingDistance -- moveIndex = $moveIndex  -- selected = $selected")
        ...
        when(event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (Math.abs(dy) > mEdgeSlop) {
                    ...
                    Logger.e(TAG, "currY = $currY  -- prevY = $prevY -- scrollY = $scrollY -- dy = $dy")
                    scrollBy(0, -dy)
                    invalidate()
                }
                prevY = currY
    ...
    private fun drawLine(canvas: Canvas) {
        var baseHeight : Float = (height - eachItemHeight) / 5.toFloat()
        paint.color = resources.getColor(R.color.graytext)
    ...
    private fun drawText(canvas: Canvas) {
        paint.color = resources.getColor(R.color.common_black)
        var y : Float
        var x : Float
        var minStart: Int = 0
        ...
        for (i in minStart until count step 1) {
            ...
            y = wheelViewMode.getTextDrawY(height/2, i, paint)
    ...
    private fun getShadowsHeight() : Int {
        return (height - eachItemHeight) /4
    }

    private fun drawShadows(canvas: Canvas) {
        val height = getShadowsHeight()
		topShadow?.setBounds(0, scrollY/4, width, (height + scrollY)/3)
        topShadow?.draw(canvas)

        bottomShadow?.setBounds(0, scrollY/4 + eachItemHeight + height, width, (getHeight() + scrollY)/3)
